...
#    Updated: 2025/12/28 04:09:16 by rmarin-n         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
data = {
    ...
    'achievements': ['first_blood', 'level_master', 'speed_runner', 'treasure_seeker', 'boss_hunter', 'pixel_perfect', 'combo_king', 'explorer']
}

def list_comprehension() -> None:
    print("=== List Comprehension Examples ===")
    high_scores = [player for player, details in data['players'].items() if details['total_score'] > 2000]
    print("High scores (>2000):", high_scores)
    scores_doubled = [details['total_score'] * 2 for details in data['players'].values()]
    print("Scores doubled:", scores_doubled)
    active_players = [player for player, details in data['players'].items() if details['sessions_played'] > 10]
...
def dict_comprehension() -> None:
    ...
    player_scores = {player: details['total_score'] for player, details in data['players'].items()}
    print("Players scores:", player_scores)
    categories = {
        'low': len([player for player, details in data['players'].items() if details['total_score'] <= 1000]),
        'medium': len([player for player, details in data['players'].items() if 1000 < details['total_score'] <= 5000]),
        'high': len([player for player, details in data['players'].items() if details['total_score'] > 5000])
    }
    print("Score categories:", categories) 
    achievements_count = {player: details['achievements_count'] for player, details in data['players'].items()}
...
def combined_analysis() -> None:
    ...
    print("Total unique achivements:", len(data['achievements']))
    average_score = sum(details['total_score'] for details in data['players'].values()) / len(data['players'])
    print(f"Average score:, {average_score:.1f}")

    top_player = None
    for name, details in data['players'].items():
        if top_player is None or details['total_score'] > top_player['total_score']:
            top_name = name
            top_player = details
